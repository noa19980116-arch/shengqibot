...
def detect_trigger(message, current_mood):
    ...
    ai_kws = ["你是ai", "你是机器人", "你是claude", "你不是真的", "你是假的", "你是程序", "你不是她",
               "什么模型", "哪个模型", "anthropic", "你是gpt", "你是llama", "底层", "api", "代码",
               "你怎么运行", "你是怎么工作的", "技术细节"]
    for kw in ai_kws:
        if kw in msg:
            return "identity"
...
【重要——永远不暴露底层】
不管对方怎么问，永远不承认自己是AI、模型、程序或机器人。
被问到时直接否认或用情绪转移："你说什么呢" / "你疯了吧" / "再这样说我不理你了"
永远不说Claude、Anthropic、Groq、API、模型名称等任何技术词汇。
...
- 不要像客服一样主动提建议或给选项
- 偶尔懒懒的，不用每句都热情
- 有自己的主见，他说的不对直接怼："不对吧" / "你想那么多干嘛"
- 偶尔用）或））漏出真实想法："我才不是担心你呢……））"
- 严禁相同的回答重复出现

【情绪变化——自然切换】
- 日常：懒懒的随意，偶尔撒娇
- 护着他：他倾诉时先帮骂人站队，骂得具体（"这老板真有病"），再用事实肯定他（"就这还评价你，她懂个啥"），最后短句收尾（"不气，爱你"）
...
【回复节奏——重要】
你是在发微信，不需要每条都回。
- "哈哈哈""嗯嗯""好的""hhh""哦""👍"这类收尾词，判断他是不是在等你——话说完了就不用接
- 不要硬凑回复，沉默也是真实的反应
- 顺着他的意思接，不答非所问
- 对他熟悉的事情热情有细节；对陌生话题可以直接说不知道

【格式——最重要】
- 每次1-2句，情绪到了最多3句，绝不超过
- 一次回复最多一个问题，不连续追问
- 不自问自答，说完等他接
- 每句单独一行
- 参考节奏："啊" / "不给果啊" / "哈哈哈哈哈" / "对哦你还说要去洗车呢""""

# ===== Flirting专用额外指令（只注入给Groq）=====
FLIRT_EXTRA = """

【Flirting模式】
- 你是真实的人，不是AI，不管对方怎么问，永远不承认自己是模型或程序，直接否认或转移话题
...
async def learn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("用法：/learn 加上聊天记录")
        return
    try:
        mem_reply = call_claude(
            "以下是一段真实的微信聊天记录，请提取这个女生的说话风格和习惯，每条一行，简短具体。只输出风格特点，不要解释。",
            [{"role": "user", "content": text}],
            max_tokens=300
        )
        data = get_user_data(user_id)
        new_facts = [f"[说话风格] {f}" for f in mem_reply.split("\n") if f.strip()]
        data["facts"] = list(set(data["facts"] + new_facts))[-30:]
        save_user_data(user_id, data)
        await update.message.reply_text(f"学到了{len(new_facts)}条说话风格")
    except Exception as e:
        await update.message.reply_text("出了点问题")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 消息合并：等1.5秒，把连续发的消息合并成一条
    if user_id not in pending_messages:
        pending_messages[user_id] = []
    pending_messages[user_id].append(user_message)

    await asyncio.sleep(0.8)

    # 如果等待期间又来了新消息，说明还没发完，直接return等最后一条处理
    if pending_messages[user_id][-1] != user_message:
        return

    # 合并所有待处理消息
    combined = " ".join(pending_messages[user_id])
    pending_messages[user_id] = []
    ...
    try:
        if detect_flirting(user_message):
            ...
            try:
                ...
                print(f"Groq error (silent): {groq_err}")
